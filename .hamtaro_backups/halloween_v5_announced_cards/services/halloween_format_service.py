...
HALLOWEEN_TIERS: list[dict[str, Any]] = [{'id': 'S', 'label': 'S', 'decks': ['Mitsurugi', 'Memento', 'Darklord']},
 {'id': 'A',
  'label': 'A',
  'decks': ['D/D/D', 'D/D', 'Archfiend', 'Fiendsmith', 'Unchained', 'Apophis', 'Yubel', 'Hecahands', 'Azamina']},
 {'id': 'B',
  'label': 'B',
  'decks': ['K9',
            'Snake-Eye',
...
HALLOWEEN_CARD_CATALOG: dict[str, dict[str, Any]] = {'Mitsurugi': {'archetypes': ['Mitsurugi']},
 ...
 'Eyes Restrict / Relinquished': {'core_exact': ['Relinquished',
                                  ...
                                  'note': "Catalogue TCG explicite : Nightmare-Eyes Restrict est exclu tant qu'il "
                                          'reste OCG-only.'},
...
def _entry(name: str, tier: str) -> dict[str, Any]:
    return {"name":name,"slug":_slug(name),"image_url":f"/static/halloween/decks/{_image_slug(name)}.jpg","tier":tier,"overrides":deepcopy(HALLOWEEN_BANLIST_OVERRIDES.get(name,[]))}
...
class HalloweenFormatService:
    def public_data(self) -> dict[str, Any]:
        ...
        catalog_payload={"catalog":HALLOWEEN_CARD_CATALOG,"overrides":HALLOWEEN_BANLIST_OVERRIDES,"global_bans":HALLOWEEN_GLOBAL_BANS,"representative_cards":HALLOWEEN_REPRESENTATIVE_CARDS,"tiers":HALLOWEEN_TIERS,"staples":HALLOWEEN_STAPLES}
        return {
            "id":"halloween","name":"Halloween","emoji":"🎃","format_version":"4.0",
            "description":"Format Halloween Hamtaro : whitelist enrichie, artworks HD, tier list temporaire, banlist spéciale et mécanique Bonbon / Sort.",
            "meta_left":"Whitelist Halloween V4","meta_right":f"{sum(len(t['decks']) for t in HALLOWEEN_TIERS)} decks + {len(HALLOWEEN_STAPLES)} groupes de staples",
    ...
    def banlist_text(self) -> str:
        lines=["FORMAT HALLOWEEN V4 - BANLIST SPÉCIALE","Toute carte absente suit la banlist TCG normale.","","[INTERDITES GLOBALEMENT]"]
