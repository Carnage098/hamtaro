(() => {
  ...
  const LOCKED_DECK_ARTWORKS = new Set(["Ashtra", "Umbral Horror", "Spirit Message", "Clown Crew", "Nemleria", "K9", "Invoked", "D/D/D", "Magical Musket"]);
  try {
  ...
  const exportTierButton = document.getElementById("halloween-tier-export");
  ...
  const wireTierBoard = () => {
    ...
    tierBoard.querySelectorAll(".halloween-live-tier-tile").forEach((tile) => {
      const tierImage = tile.querySelector("img");
      if (tierImage && !tierImage.dataset.localTierSrc) {
        tierImage.dataset.localTierSrc = tierImage.getAttribute("src") || "";
      }
  ...
  // ------------------------------------------------------------
  // Export PNG HD de la tier list courante
  // ------------------------------------------------------------
  ...
  const exportWrapText = (ctx, text, maxWidth, maxLines = 3) => {
  ...
  const waitForExportImages = async (images) => {
    const loaded = new Map();
    await Promise.all(images.map(async ({key, src}) => {
      if (!src || loaded.has(key)) return;
      loaded.set(key, await loadExportImage(src));
    }));
    return loaded;
  };

  const exportHalloweenTierList = async () => {
    if (!tierBoard || !exportTierButton) return;

    const previousText = exportTierButton.textContent;
    exportTierButton.classList.add("is-exporting");
    exportTierButton.disabled = true;
    exportTierButton.textContent = "⏳ Création du PNG…";

    try {
      const rows = [...tierBoard.querySelectorAll(".halloween-live-tier-row")].map((row) => ({
        id: row.dataset.tierRow || "",
        label: row.querySelector(".halloween-live-tier-label")?.textContent?.trim() || row.dataset.tierRow || "",
        cards: [...row.querySelectorAll(".halloween-live-tier-tile")].map((tile) => {
          const image = tile.querySelector("img");
          return {
            name: tile.dataset.deckName || tile.querySelector("strong")?.textContent?.trim() || "Deck",
            imageSrc: image?.dataset.localTierSrc || image?.getAttribute("src") || "",
            release: tile.querySelector(".halloween-release-badge")?.textContent?.trim() || "",
          };
        }),
      }));
      ...
      const cardWidth = 154;
      const artHeight = 154;
      const cardTextHeight = 54;
      const cardHeight = artHeight + cardTextHeight;
      const gap = 12;
      const usableWidth = logicalWidth - (margin * 2) - labelWidth - (rowPadding * 2) - gap;
      const cardsPerLine = Math.max(1, Math.floor((usableWidth + gap) / (cardWidth + gap)));

      const rowLayouts = rows.map((row) => {
        const lines = Math.max(1, Math.ceil(row.cards.length / cardsPerLine));
        const height = rowPadding * 2 + lines * cardHeight + Math.max(0, lines - 1) * gap;
        return {...row, lines, height};
      });

      const logicalHeight = headerHeight + footerHeight + rowLayouts.reduce((sum, row) => sum + row.height + 3, 0) + margin;
      const scale = 2;
      ...
      const neededImages = rows.flatMap((row) => row.cards.map((card) => ({key: `${row.id}:${card.name}`, src: card.imageSrc})));
      const loadedImages = await waitForExportImages(neededImages);

      // Fond général
      const bg = ctx.createLinearGradient(0, 0, logicalWidth, logicalHeight);
      bg.addColorStop(0, "#120b18");
      bg.addColorStop(0.55, "#171019");
      ...
      ctx.fillText("HAMTARO · TIER LIST HALLOWEEN", margin, 72);
      ctx.fillStyle = "rgba(255,255,255,.66)";
      ctx.font = "500 22px system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
      ctx.fillText("Tier list personnalisée · export PNG", margin, 106);

      let y = headerHeight;
      for (const row of rowLayouts) {
        // panneau de rangée
        ctx.fillStyle = "rgba(255,255,255,.045)";
        ctx.fillRect(margin, y, logicalWidth - margin * 2, row.height);

        // label du tier
        ctx.fillStyle = tierExportColors[row.id] || "#555";
        ctx.fillRect(margin, y, labelWidth, row.height);
        ctx.save();
        ctx.translate(margin + labelWidth / 2, y + row.height / 2);
        ctx.fillStyle = "#fff";
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.font = "900 32px system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
        ctx.fillText(row.label.toUpperCase(), 0, 0);
        ctx.restore();

        row.cards.forEach((card, index) => {
          const col = index % cardsPerLine;
          const line = Math.floor(index / cardsPerLine);
          const x = margin + labelWidth + gap + rowPadding + col * (cardWidth + gap);
          const cardY = y + rowPadding + line * (cardHeight + gap);
          ...
          const image = loadedImages.get(`${row.id}:${card.name}`);
          if (image) {
            ctx.save();
            roundedRect(ctx, x, cardY, cardWidth, artHeight, 10);
            ctx.clip();
            const iw = image.naturalWidth || image.width;
            const ih = image.naturalHeight || image.height;
            const sourceSide = Math.min(iw, ih);
            const sx = Math.max(0, (iw - sourceSide) / 2);
            const sy = Math.max(0, (ih - sourceSide) / 2);
            ctx.drawImage(image, sx, sy, sourceSide, sourceSide, x, cardY, cardWidth, artHeight);
            ctx.restore();
          } else {
            ctx.fillStyle = "#332b38";
            ctx.fillRect(x, cardY, cardWidth, artHeight);
          }

          // léger dégradé pour séparer l'art du nom
          const cardGradient = ctx.createLinearGradient(0, cardY + artHeight - 20, 0, cardY + artHeight + 4);
          cardGradient.addColorStop(0, "rgba(20,18,24,0)");
          cardGradient.addColorStop(1, "rgba(20,18,24,.95)");
          ctx.fillStyle = cardGradient;
          ctx.fillRect(x, cardY + artHeight - 20, cardWidth, 24);

          ctx.fillStyle = "#fff";
          ctx.font = "700 15px system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
          ctx.textAlign = "left";
          ctx.textBaseline = "top";
          const lines = exportWrapText(ctx, card.name, cardWidth - 18, 2);
          lines.forEach((lineText, lineIndex) => {
            ctx.fillText(lineText, x + 9, cardY + artHeight + 9 + lineIndex * 18);
          });

          if (card.release) {
            ctx.fillStyle = "rgba(255,166,70,.95)";
            ctx.font = "800 11px system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
            ctx.textAlign = "right";
            ctx.fillText(card.release, x + cardWidth - 8, cardY + cardHeight - 16);
          }
        });
      ...
      ctx.textAlign = "right";
      ctx.fillText("Hamtaro · JJETGAMES", logicalWidth - margin, logicalHeight - 26);
      ...
      link.href = url;
      link.download = `hamtaro-halloween-tier-list-${date}.png`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.setTimeout(() => URL.revokeObjectURL(url), 1500);
    } catch (error) {
      console.error("Export PNG Halloween impossible", error);
      window.alert("Impossible de créer le PNG de la tier list. Réessaie après le chargement complet des artworks.");
    } finally {
      exportTierButton.disabled = false;
      exportTierButton.classList.remove("is-exporting");
      exportTierButton.textContent = previousText || "📸 Enregistrer en PNG";
    }
  };

  exportTierButton?.addEventListener("click", exportHalloweenTierList);
