...
from services.meta_center_v2_service import MetaCenterV2Service
...
_REGISTER_FLAG = "hamtaro_competitive_v2_routes"


def register_competitive_v2_routes(
    ...
    meta = getattr(site, "meta_center_v2", None) or MetaCenterV2Service()
    live = getattr(site, "tournament_live_v2", None) or TournamentLiveService()
    identity = getattr(site, "player_identity_v2", None) or PlayerIdentityService()
    site.meta_center_v2 = meta
    site.tournament_live_v2 = live
    site.player_identity_v2 = identity
    ...
    application.router.add_get("/api/v2/meta", meta_api)
    application.router.add_get(r"/api/v2/meta/{deck:.+}", meta_deck_api)
    application.router.add_get(
        r"/api/v2/players/{discord_id:\d+}/identity", identity_api
    )
    ...
    application.router.add_get(
        r"/api/v2/tournaments/{tournament_id:\d+}/live", live_center_api
    )
    application.router.add_get(
