(() => {
    ...
    const refreshMilliseconds = Math.max(
        5,
        Number.isFinite(configuredSeconds) ? configuredSeconds : 15
    ) * 1000;

    const updateLabel = document.querySelector("[data-staff-last-update]");
    const liveDot = document.querySelector(".staff-live-dot");
    let running = false;

    const text = (value, fallback = "") => String(value ?? fallback);

    const replaceRows = (selector, rows, emptyMessage, columnCount) => {
    ...
    const replaceCards = (selector, rows, emptyMessage, builder) => {
        ...
        rows.forEach((item) => container.append(builder(item)));
    };
    ...
    const refresh = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                markStatus(false, "Mise à jour momentanément indisponible");
                return;
            }
            ...
            statValues.forEach((element, index) => {
                element.textContent = text(orderedTotals[index], 0);
            });
            ...
            replaceCards(
                ...
                (item) => {
                    ...
                    article.className = "staff-issue";
                    const strong = document.createElement("strong");
                    strong.textContent = `${text(item.tournament_code)} · Match #${text(item.id)}`;
                    const span = document.createElement("span");
                    span.textContent = `${text(item.player1_name, "?")} contre ${text(item.player2_name, "?")}`;
                    const code = document.createElement("code");
            ...
            replaceCards(
                ...
                (item) => {
                    const article = document.createElement("article");
                    article.className = "staff-audit-entry";
                    const wrapper = document.createElement("div");
                    const strong = document.createElement("strong");
                    strong.textContent = text(item.action, "Action");
                    ...
                    date.textContent = text(item.created_at);
                    wrapper.append(strong, actor);
                    article.append(wrapper, date);
                    return article;
                }
            );
            ...
            markStatus(false, "Connexion au tableau de bord interrompue");
        } finally {
            running = false;
        }
    };
