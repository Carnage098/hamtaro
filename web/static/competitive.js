(() => {
  ...
  async function loadMeta(period = "30d") {
    ...
    const url = "/api/competitive/meta?" + new URLSearchParams({
      period,
      ...(format ? { format } : {})
    }).toString();
    ...
    if (podium) {
      podium.innerHTML = (data.podium || []).map((row, index) => `
        <div class="competitive-list-row">
      ...
      emerging.innerHTML = (data.emerging || []).map((row, index) => {
        ...
        return `
          <div class="competitive-list-row">
            ...
            <span class="competitive-delta ${delta >= 0 ? "up" : "down"}">${delta >= 0 ? "+" : ""}${delta.toFixed(1)} pt</span>
          </div>
        `;
    ...
    document.querySelectorAll("[data-meta-deck]").forEach((node) => {
      ...
      const sample = row.sample_label
        ? `<span class="competitive-chip warn">⚠ ${esc(row.sample_label)}</span>`
        : "";
      node.innerHTML = `
        <span class="competitive-chip">${Number(row.popularity || 0).toFixed(1)}% méta</span>
        <span class="competitive-chip ${delta >= 0 ? "" : "warn"}">${delta >= 0 ? "↗" : "↘"} ${Math.abs(delta).toFixed(1)} pt</span>
        ${sample}
      `;
    });
  }
  ...
  async function enrichPlayer() {
    ...
    if (!match) return;
    const response = await fetch(`/api/competitive/players/${match[1]}/identity`, { cache: "no-store" });
    if (!response.ok) return;
    const data = await response.json();
    ...
    panel.className = "identity-competitive";
    panel.innerHTML = `
      <span class="competitive-kicker">IDENTITÉ HAMTARO</span>
