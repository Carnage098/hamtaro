...
_PATCH_MARKER = "hamtaro-staff-dashboard-v2"
...
def _enabled() -> bool:
    return _truthy(os.getenv("STAFF_DASHBOARD_ENABLED"), default=False)
...
def _session_digest(secret: str) -> str:
    return hmac.new(
        secret.encode("utf-8"),
        b"hamtaro-staff-dashboard-session-v2",
        hashlib.sha256,
    ).hexdigest()
...
def _authorized(request: web.Request) -> bool:
    ...
    expected = _session_digest(secret)
    return bool(supplied) and hmac.compare_digest(supplied, expected)


async def _record_web_audit(
    self,
    *,
    action: str,
    ...
    try:
        ...
        # Le tableau de bord reste accessible même si l'audit est indisponible.
        LOGGER.exception(
...
async def _staff_page(self, request: web.Request) -> web.Response:
    ...
    if len(secret) < 24:
        return self.render(
            ...
            error=(
                ...
                "Définis STAFF_DASHBOARD_TOKEN avec au moins 24 caractères."
            ),
            status_code=503,
        )
    ...
    if guild_id is None:
        return self.render(
            ...
            message=(
                "Aucun GUILD_ID ou PUBLIC_GUILD_ID valide n'est configuré."
            ),
            status_code=503,
        )
    ...
    try:
        refresh_seconds = max(
            5,
            int(os.getenv("LIVE_SITE_REFRESH_SECONDS", "15") or "15"),
        )
    except ValueError:
        refresh_seconds = 15
...
async def _staff_login(self, request: web.Request) -> web.Response:
    ...
    attempts: dict[str, deque[float]] | None = getattr(
        self,
        "_staff_dashboard_login_attempts",
        None,
    )
    if attempts is None:
        attempts = defaultdict(deque)
        self._staff_dashboard_login_attempts = attempts
...
async def _patched_start_server(self) -> None:
    ...
    application.router.add_get(
        "/api/tournaments/live.json",
        self.live_tournaments_api,
    )
    application.router.add_get(
    ...
    application.router.add_post("/staff/logout", self.staff_logout)
    # Compatibilité avec l'ancien bouton de déconnexion en lien GET.
    application.router.add_get("/staff/logout", self.staff_logout)
    application.router.add_get(
        "/staff/api/overview",
        self.staff_overview_api,
    )
    ...
    try:
        ...
        LOGGER.exception(
            "Impossible de lancer le site Hamtaro sur %s:%s.",
            host,
            port,
        )
        raise
    ...
    self.site = site
    LOGGER.info(
        "Site Hamtaro avec tableau de bord staff lancé sur %s:%s.",
        host,
        port,
    )


def apply_patch() -> type:
    from cogs.public_website import PublicWebsiteCog

    if getattr(PublicWebsiteCog, "_staff_dashboard_patch", None) == _PATCH_MARKER:
        return PublicWebsiteCog

    PublicWebsiteCog._staff_dashboard_patch = _PATCH_MARKER
    PublicWebsiteCog._start_server = _patched_start_server
    PublicWebsiteCog._security_headers_middleware = _security_headers_middleware
...
class ProfessionalWebPatchCog(commands.Cog):
    """Charge les routes staff avant ou après le site public."""
...
async def setup(bot: commands.Bot) -> None:
    public_website_class = apply_patch()

    # Le chargement avant cogs.public_website est recommandé. Si l'extension
    # est néanmoins chargée après, on redémarre proprement le serveur afin que
    # les nouvelles routes soient enregistrées et que /staff ne renvoie plus 404.
    existing_cog = bot.get_cog(public_website_class.__name__)
    if existing_cog is not None and existing_cog.runner is not None:
        LOGGER.warning(
            "professional_web a été chargé après public_website : "
            "redémarrage contrôlé du serveur web."
        )
        await existing_cog._stop_server()
        await existing_cog._start_server()

    await bot.add_cog(ProfessionalWebPatchCog(bot))
